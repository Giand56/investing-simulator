...
async def get_stock_history(ticker: str, period: str) -> dict:
    def _fetch():
        ...
        data = [
            {
                "date": index.strftime("%b %-d"),  # → "Jan 1", "Mar 15" etc.
...
async def get_previous_close(ticker: str) -> float:
    def _fetch():
        stock = yf.Ticker(ticker)
...
async def get_daily_pl_percent(ticker) -> float | None:
    current_price = await get_stock_price(ticker)
    prev_close = await get_previous_close(ticker)
    daily_change_per_share = (current_price - prev_close) / prev_close * 100
    return round(daily_change_per_share, 2)


async def get_daily_pl(ticker) -> float | None:
    current_price = await get_stock_price(ticker)
    prev_close = await get_previous_close(ticker)
    daily_change_per_share = current_price - prev_close
    return round(daily_change_per_share, 2)

async def get_stock_stats(ticker: str) -> dict:
    def _fetch():
        stock = yf.Ticker(ticker)
        info = stock.info
